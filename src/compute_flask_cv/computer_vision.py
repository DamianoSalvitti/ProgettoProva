...
def rgb2gray(file_path):
    ...
    np_array = cv2.imwrite(new_path, gray_scale)
    
    return gray_scale, np_array


# Convert from RGB to BGR
def rgb2bgr(file_path):
    # Read, convert
    img = cv2.imread(file_path)
    bgr_scale = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

    # Write
    f_path = os.path.dirname(__file__)
    new_path = os.path.join(f_path, os.pardir, "A_Cannone_bgr.jpg")
    np_array = cv2.imwrite(new_path, bgr_scale)

    return bgr_scale, np_array
